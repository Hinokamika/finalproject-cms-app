...
export default function MealPlansTable() {
  ...
  const [filters, setFilters] = useState<{ search?: string; start_date?: string; end_date?: string; user_id?: string }>({});
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/meal-plans?${queryString}`);
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/meal-plans/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  const columns: ColumnsType<Rec> = [
    ...
    { title: "User", dataIndex: "user_id", width: 220 },
    { title: "Day", dataIndex: "day_label", width: 180 },
    { title: "Meals", dataIndex: "meals", render: (v) => Array.isArray(v) ? v.length : 0, width: 100 },
    { title: "Snacks", dataIndex: "snacks", render: (v) => Array.isArray(v) ? v.length : 0, width: 100 },
    ...
    <div className="bg-white p-4 rounded-lg shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex gap-2 items-center flex-wrap">
          <Input.Search allowClear placeholder="Search day label" onSearch={(v)=>{ setPage(1); setFilters((f)=>({ ...f, search: v || undefined })); }} style={{ width: 280 }} />
      ...
      <MealPlanModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
