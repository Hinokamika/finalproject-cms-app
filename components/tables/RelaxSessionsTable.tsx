...
import RelaxSessionModal from "@/lib/components/modals/RelaxSessionModal";
import dayjs from "dayjs";

type Rec = any;

export default function RelaxSessionsTable() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/relax-sessions?${queryString}`);
  ...
  const onDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/relax-sessions/${id}`, { method: "DELETE" });
      const json = await res.json();
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/relax-sessions/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  const columns: ColumnsType<Rec> = [
    ...
    { title: "Started", dataIndex: "started_at", render: (v) => dayjs(v).format("YYYY-MM-DD HH:mm"), width: 180 },
    { title: "Minutes", dataIndex: "minutes", width: 100 },
    { title: "Pattern", dataIndex: "pattern", width: 140 },
    { title: "Mood", dataIndex: "mood", width: 140 },
    {
      title: "Actions",
      key: "actions",
      width: 120,
    ...
    <div className="bg-white p-4 rounded-lg shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <DatePicker.RangePicker showTime onChange={(values) => {
      ...
      <RelaxSessionModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
