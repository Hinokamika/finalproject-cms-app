...
import ChatMessageModal from "@/lib/components/modals/ChatMessageModal";
...
export default function ChatMessagesTable() {
  ...
  const [filters, setFilters] = useState<{ search?: string; start_date?: string; end_date?: string; user_id?: string; role?: string }>({});
  ...
  const queryString = useMemo(() => {
    ...
    if (filters.role) params.set("search", filters.role); // simple filter via search
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/chat-messages?${queryString}`);
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/chat-messages/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex gap-2 items-center flex-wrap">
          <Space.Compact>
            <Input
              allowClear
              placeholder="Search content/role"
              style={{ width: 240 }}
      ...
      <ChatMessageModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
