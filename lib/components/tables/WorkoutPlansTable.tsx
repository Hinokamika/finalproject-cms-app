"use client";
import { Table, Space, Popconfirm, Tooltip, Button, Input, DatePicker, Select, notification } from "antd";
import { SearchOutlined } from "@ant-design/icons";
import type { ColumnsType } from "antd/es/table";
import { useEffect, useMemo, useState } from "react";
import { EditOutlined, DeleteOutlined, PlusOutlined, ReloadOutlined } from "@ant-design/icons";
import WorkoutPlanModal from "@/lib/components/modals/WorkoutPlanModal";
...
export default function WorkoutPlansTable() {
  ...
  const [filters, setFilters] = useState<{ search?: string; start_date?: string; end_date?: string; user_id?: string; level?: string }>({});
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/workout-plans?${queryString}`);
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/workout-plans/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  const columns: ColumnsType<Rec> = [
    ...
    { title: "Exercise", dataIndex: "exercise_name" },
    { title: "Category", dataIndex: "exercise_category", width: 160 },
    { title: "Level", dataIndex: "difficulty_level", width: 120 },
    { title: "Duration", dataIndex: "duration_minutes", width: 110 },
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <Space.Compact>
            <Input
              ...
              onPressEnter={(e)=> { setPage(1); setFilters((f)=> ({ ...f, search: (e.target as HTMLInputElement).value || undefined })); }}
            />
      ...
      <WorkoutPlanModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
