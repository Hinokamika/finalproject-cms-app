...
export default function NutritionIntakeTable() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/nutrition-intake?${queryString}`);
  ...
  const onDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/nutrition-intake/${id}`, { method: "DELETE" });
      const json = await res.json();
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/nutrition-intake/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  const columns: ColumnsType<Rec> = [
    ...
    { title: "kcal", dataIndex: "energy_kcal", width: 100 },
    { title: "Carbs", dataIndex: "carbs_g", width: 100 },
    { title: "Protein", dataIndex: "protein_g", width: 100 },
    { title: "Fat", dataIndex: "fat_g", width: 100 },
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
      ...
      <NutritionIntakeModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
