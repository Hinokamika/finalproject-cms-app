...
import RelaxPlanModal from "@/lib/components/modals/RelaxPlanModal";
...
export default function RelaxPlansTable() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/relax-plans?${queryString}`);
  ...
  const onDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/relax-plans/${id}`, { method: "DELETE" });
      const json = await res.json();
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/relax-plans/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  const columns: ColumnsType<Rec> = [
    ...
    { title: "Type", dataIndex: "relaxation_type", width: 160 },
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex gap-2 items-center flex-wrap">
          <Space.Compact>
            <Input
              allowClear
              placeholder="Search title/type/day"
              style={{ width: 240 }}
      ...
      <RelaxPlanModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
