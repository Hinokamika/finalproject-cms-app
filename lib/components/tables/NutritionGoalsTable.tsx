...
export default function NutritionGoalsTable() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/nutrition-goals?${queryString}`);
  ...
  const onDelete = async (id: number) => {
    try {
      const res = await fetch(`/api/nutrition-goals/${id}`, { method: "DELETE" });
      const json = await res.json();
  ...
  const onBulkDelete = async () => {
    try {
      const res = await fetch(`/api/nutrition-goals/bulk`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: selectedRowKeys }) });
      const json = await res.json();
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
      ...
      <NutritionGoalModal open={modalOpen} onClose={() => setModalOpen(false)} initialValues={editing} onSuccess={load} />
    </div>
  );
}
