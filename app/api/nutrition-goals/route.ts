...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseAdmin.from("nutrition_goal_history").select("*", { count: "exact" }).range(from, to).order("created_at", { ascending: false });
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("nutrition_goal_history").insert(body).select("*").single();
    if (error) throw error;
