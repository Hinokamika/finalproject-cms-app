...
export async function GET(req: Request) {
  try {
    ...
    if (search) {
      ...
      query = query.or(`title.ilike.${term},description.ilike.${term},relaxation_type.ilike.${term},day.ilike.${term}`);
    }
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("relax_plan").insert(body).select("*").single();
    if (error) throw error;
