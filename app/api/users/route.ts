...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseAdmin.from("user_identity").select("*", { count: "exact" }).range(from, to).order("created_at", { ascending: false });
    ...
    if (search) {
      ...
      query = query.or(`user_name.ilike.${term},email.ilike.${term},phone.ilike.${term}`);
    }
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("user_identity").insert(body).select("*").single();
    if (error) throw error;
