...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseAdmin.from("meal_plan").select("*", { count: "exact" }).range(from, to).order("created_at", { ascending: false });
    ...
    if (search) {
      ...
      query = query.or(`day_label.ilike.${term}`);
    }
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("meal_plan").insert(body).select("*").single();
    if (error) throw error;
